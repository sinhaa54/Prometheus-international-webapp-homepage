...
export function isTableauUrlSafe(url: string | null | undefined, allowedHosts: string[]): boolean {
  if (!url) return false;
  let parsed: URL;
  try {
    parsed = new URL(url);
 ...
 */
export function openTableauDashboard(
  url: string | null | undefined,
  opts: { allowedHosts: string[]; openInNewTab: boolean },
): { ok: boolean; error?: string } {
  if (!url) return { ok: false, error: 'This dashboard has no Tableau URL configured.' };
